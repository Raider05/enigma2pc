...
#			"right": self.keyRight,	# KEY_RIGHT
	...
	def updateEntryList(self):
		...
		if not config.usage.bouquethotkeys.value:
			list = [self.setEntryComponent("icons/lock_off", "Enable bouquet hotkeys", 0)]
		else:
			if len(self.list):
				idx = self["entrylist"].index
			serviceHandler = eServiceCenter.getInstance()
			list = [self.setEntryComponent("icons/lock_on", "Disable bouquet hotkeys", 0)]
			for x in range(1, 10):
				sx = str(x)
	...
	def keyOk(self):
		...
		if idx == 0:
			...
			if bouquets:
				...
				self.session.open(MessageBox, "Sorry, but bouquets not found :(")
